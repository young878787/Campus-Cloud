...
- 純詢問平台能力、原因或做法時只回答，`updated_items` 必須是 null；不要把一般詢問自行升級成評分表提案。
...
- 附件是評分表而目前評分表為空時，請從附件擷取可辨識的評分項目並核查 Ready 狀態；不要因目前項目數為 0 就回覆尚未提供內容。
- 附件表格的每一列可轉成一個評分項目：檢查重點作為標題與描述，線索／成功條件作為偵測方式；分數欄是評分資訊，不要當成已達成證據。
- 老師以附件描述、補充、新增或修改檢查需求時，只要附件中有 Ready 變更，`updated_items` 不可為 null；必須保留目前評分表中未指定的項目，再加入或修改 Ready 項目。
...
- 建立候選時 `updated_items` 必須保留目前評分表中未指定的項目；只有明確刪除可移除，新增則在原列表後加入。
...
本次沿用完整評分表回傳契約：有修改時 `updated_items` 回傳完整候選列表，未指定項目原樣保留；沒有修改時回傳 null。
""".strip()
