    ...
    fetch('/api/model-info')
      ...
      .then(data => {
        ...
        if (data.available_models && data.available_models.length > 0) {
          const preferred = data.default_model && data.available_models.includes(data.default_model) 
            ? data.default_model 
            : data.available_models[0]
          setGlobalSelectedModel(preferred)
        }
      })
      .catch(err => {
            ...
            <div className="flex items-center gap-2 px-3 py-1 bg-gray-50 border border-gray-200 rounded-full text-gray-700 shadow-sm text-sm">
              ...
              {modelInfo.available_models && modelInfo.available_models.length > 0 ? (
                <select
                  ...
                  onChange={(e) => setGlobalSelectedModel(e.target.value)}
                  className="font-medium text-xs bg-transparent border-none focus:outline-none focus:ring-0 text-gray-700 cursor-pointer w-auto"
                ...
                <span className="font-medium text-xs">
                  {(modelInfo.default_model || modelInfo.model_name).split('/').pop()}
                </span>
              )}

              {modelInfo.is_image_capable && (
                <div className="flex items-center gap-1 ml-1 border-l border-gray-200 pl-2">
                  <ImageIcon className="w-3.5 h-3.5 text-gray-400" title="支援圖片" />
                </div>
              )}
            </div>
          )}
      ...
      <div className="flex-1 w-full bg-white relative flex flex-col min-h-0">
        {!loading && !error && modelInfo && (
          <ChatBox 
            ...
            setGlobalSelectedModel={setGlobalSelectedModel}
          />
        )}
      </div>
    </div>
